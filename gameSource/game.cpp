...
static void drawPauseScreen() {
    ...
    if( currentGamePage == livingLifePage ) {
        ...
        for(int c=1;c<3;c++) {
            ...
            if( p == 1 && !backgroundDrawn ) {
                ...
                drawRect( lastScreenViewCenter, HetuwMod::viewWidth, HetuwMod::viewHeight );
                stopStencil();
            ...
            if( helpFileContents != NULL ) {
                ...
                for( int i=0; i<numLines; i++ ) {
                    ...
                    if ( isComment ) {
                        ...
                        titleFont->drawString( lines[i], { writePos.x + 40, writePos.y - lineHeight }, alignLeft ); // Left-align
                        writePos.y -= lineHeight * 0.75f*3;
                        }
                    else if ( isSub ) {
                        setDrawColor( 0.2f, 0.4f, 0.6f, 1*pauseScreenFade );
                        handwritingFont->drawString( lines[i], { writePos.x + 40, writePos.y - lineHeight * 0.75f }, alignLeft );
                        int subSize = handwritingFont->measureString( lines[i] );
                        setDrawColor( 0.1f, 0.1f, 0.1f, 1*pauseScreenFade );
                        pencilFont->drawString( subString, { writePos.x + subSize + 60, writePos.y - lineHeight * 0.75f }, alignLeft );
    ...
    if( currentGamePage == livingLifePage ) {
        ...
        doublePair resumeButtonPos = { 460, -112 };
        doublePair settingsButtonPos = { 460, -192 };
        doublePair quitButtonPos = { 460, -272 };
        ...
        if( 1 ) { // Resume button
            ...
            if( abs(lastCursorPos.x - buttonPos.x) < subSize &&
                abs(lastCursorPos.y - buttonPos.y) < 40/2 ) {
